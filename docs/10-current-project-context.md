...
本次快照原因：完成复杂高资金量示例财务数据后，按上下文压缩恢复规则刷新。
...
- 示例数据已升级为复杂高资金量压力测试集，覆盖多银行卡、微信/支付宝、证券账户、基金账户、信用卡、固定资产、应收款、房贷、车贷、信用卡、花呗/白条、借款和跨月交易。
...
- 当前示例数据是复杂压力测试数据集。
- 覆盖 2026 年 1-4 月收入、费用、投资买卖、信用卡消费、信用卡还款、房贷/车贷还款、借款、应收款调整等交易。
- 资金量已提升到百万级资产、百万级负债和多账户结构。
- 修改 seedData 后，已安装用户需要在“我的”页执行“恢复示例数据”才能看到新数据。
